...
void LoopyGraphState::destroyAutoGeneratedInputNodes()
{
    for (size_t i = 0; i < autoGeneratedInputNodes.size(); ++i) {
        string k = autoGeneratedInputNodes[i]->outputKey;
        delete allNodes[k];
        allNodes.erase(k);
    }
}

void LoopyGraphState::readGraphFromFile(std::ifstream& ifs)
{
    json j;
    ifs >> j;
    ifs.clear();                 // clear fail and eof bits
    ifs.seekg(0, std::ios::beg); // back to the start!
    destroyAutoGeneratedInputNodes();
    parseJson(j);
    parseInputs(j);
}
