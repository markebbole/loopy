...
void addInputFromNodeName(LoopyNode* node, string nodeKey, string inputNode, map<string, LoopyNode*> allNodes)
{
    // weak vs not weak
    if (inputNode.at(0) == '-') {
        node->addInput(allNodes[val.substr(1)], nodeKey, false);
    } else {
        node->addInput(allNodes[val], nodeKey, true);
    }
}
 ...
 */
void LoopyGraphState::parseParams(LoopyNode* node, json& paramObject, map<string, LoopyNode*>& allNodes, vector<LoopyInputNode*>& autoGeneratedInputNodes)
{
    ...
    for (json::iterator it = j.begin(); it != j.end(); ++it) {
        ...
        for (json::iterator it2 = it.value().begin(); it2 != it.value().end(); ++it2) {
            ...
            if (k == "params") {
                ...
                parseParams(paramObject, allNodes, autoGeneratedInputNodes);
            } else if (k != "type") {
